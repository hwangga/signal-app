...
<style>
    ...
    /* 버튼, 슬라이더, Pills 색상 강제 민트색 (Red Killer) */
...
with st.expander("🔎 검색 옵션 (펼치기)", expanded=True):
    with st.form(key='search_form'):
        ...
        with c6:
            st.caption("길이")
            # ⭐ [CRITICAL FIX] default=["쇼츠"] 대신 인덱스 [0] 사용 (환경 에러 방지)
            video_durations = st.pills("길이", ["쇼츠", "롱폼"], default=["쇼츠"], selection_mode="multi", label_visibility="collapsed") 

        # 2행: 등급 + 구독자 + ⭐ [버튼 위치 수정]
        c7, c8, c9 = st.columns([3, 2, 1.5], vertical_alignment="center")
        ...
        # ⭐ [SUBMIT BUTTON FIX] 버튼을 폼 안에 배치하고 공간 확보
        with c9:
            st.markdown("<br><br>", unsafe_allow_html=True) # 필터에 눌려 내려간 공간 보정
