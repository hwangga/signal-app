...
# 등급 명칭
GRADE_NAMES = ["🚀 떡상중 (1000%↑)", "📈 급상승 (300%↑)", "👀 주목 (100%↑)", "💤 일반 (100%↓)"]
...
st.markdown("""
<style>
    /* 전체 테마 및 버튼 스타일 CSS (이전과 동일) */
    .stApp { background-color: #0E1117; color: #FAFAFA; }
    section[data-testid="stSidebar"] { min-width: 450px !important; background-color: #1A1C24; border-right: 1px solid #333; text-align: center; }
    ...
    /* 태그, 슬라이더, 메트릭 숫자 색상 유지 */
    [data-testid="stPills"] button[aria-pressed="true"] { background-color: #00E5FF !important; color: black !important; }
    ...
    .sidebar-logo {
        ...
        border: 2px solid #00E5FF; /* 테두리 강조 */
        width: 90%;
    }
...
with st.expander("🔎 검색 옵션 (펼치기)", expanded=True):
    with st.form(key='search_form'):
        ...
        # 1행: 키워드 | 수집 | 기간 | 국가
        c1, c2, c3, c4 = st.columns([1.5, 0.7, 0.8, 1.5], vertical_alignment="bottom")
        with c1: query = st.text_input("키워드", placeholder="키워드 입력")
        with c2: max_results = st.selectbox("수집", [10, 30, 50, 100], index=1)
        ...
        # 2행: 길이 | 등급 | 카테고리 | 구독자 범위
        c5, c6, c7, c8 = st.columns([1.2, 2.5, 2.5, 2.5], vertical_alignment="center")
        with c5: 
            st.caption("길이")
            video_durations = st.pills("길이", ["쇼츠", "롱폼"], default=["шо츠"], selection_mode="multi", label_visibility="collapsed")
        with c6: 
            st.caption("등급 필터")
        ...
        with c7:
            # ⭐ [새 기능] 카테고리 필터
            st.caption("주제 (카테고리)")
            category_name = st.selectbox("카테고리", list(CATEGORY_MAP.keys()), index=0, label_visibility="collapsed")
        with c8: 
            st.caption("구독자 범위")
            subs_range = st.slider("구독자", 0, 1000000, (0, 1000000), 1000, label_visibility="collapsed")

        # 3행: 고급 필터 및 검색 버튼
        col_adv, col_search = st.columns([2, 1])
        with col_adv:
            # ⭐ [새 기능] 연령 제한 필터
            age_filter = st.checkbox("연령 제한(성인) 콘텐츠 제외", value=True, help="유튜브의 연령 제한 콘텐츠(ytRating)를 자동으로 검색 결과에서 제외합니다.")
        with col_search:
...
category_id = CATEGORY_MAP.get(category_name) # 카테고리 ID
        ...
        try:
            ...
            with st.spinner(f"📡 '{query}' 신호 분석 중..."):
                ...
                if not all_video_ids: 
                    ...
                    items = []
                    for chunk in chunks:
                        # contentDetails (길이, 연령 제한), statistics, snippet, contentDetails 모두 요청
                    ...
                    for chunk in channel_chunks:
                        # statistics (구독자), contentDetails (총 영상 수) 요청
                        ...
                        for item in channel_response['items']:
                            ...
                            video_count_map[item['id']] = int(item['statistics'].get('videoCount', 0)) # ⭐ [새 기능] 총 영상 수

                    raw_data_list = []
                    for item in items:
                        # ⭐ [필터] 연령 제한 필터링
                        if age_filter and item['contentDetails'].get('contentRating', {}).get('ytRating') in ['ytAgeRestricted']:
                             continue # 연령 제한 콘텐츠 제외

                        # ⭐ [필터] 카테고리 필터링
                        if category_id is not None and item['snippet']['categoryId'] != category_id:
                            continue
                        ...
                        daily_velocity = view_count / days_passed # ⭐ [새 기능] 일일 평균 클릭 계산
                        ...
                        raw_data_list.append({
                            "raw_perf": perf, "raw_date": published_date, "raw_view": view_count, "raw_sub": sub_count, 
                            ...
                            # ⭐ [새 지표]
                            "총 영상": video_count_map.get(item['snippet']['channelId'], 0),
                            "일일 속도": int(daily_velocity)
                        })
                    ...
                    for i, row in enumerate(sorted_list):
                        display_data.append({
                            ...
                            "게시일": row['raw_date'].strftime("%Y/%m/%d"), "총 영상": f"{row['총 영상']:,}개", # 표시용
                            "구독자": f"{row['raw_sub']:,}", "조회수": f"{row['raw_view']:,}",
                            "성과도": row['raw_perf'], "등급": row['grade'], "길이": row['duration'],
                            "일일 속도": f"{row['일일 속도']:,}", # ⭐ [새 지표] 테이블에 추가
...
with st.sidebar:
    ...
    if st.session_state.df_result is not None and not st.session_state.df_result.empty:
        ...
        m2.metric("S-Tier", f"{s_tier_count}개")
...
if st.session_state.df_result is not None:
    ...
    selection = st.dataframe(
        df,
        column_order=("No", "썸네일", "채널명", "제목", "게시일", "총 영상", "구독자", "조회수", "성과도", "등급", "일일 속도", "길이", "이동"),
        column_config={
            ...
            "게시일": st.column_config.TextColumn("게시일", width=75),
            "총 영상": st.column_config.TextColumn("총 영상", width=70, help="채널의 총 영상 개수"), # ⭐ [새 컬럼]
            "구독자": st.column_config.TextColumn("구독자", width=75),
            ...
            "일일 속도": st.column_config.TextColumn("일일 속도", width=80, help="업로드 후 하루 평균 조회수"), # ⭐ [새 컬럼]
